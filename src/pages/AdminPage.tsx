...
const UEBUNGEN_FIELDS = [
  ...
  { key: 'created_at', label: 'Erstellt am', type: 'date/date' },
];
const PREINTRAEGE_FIELDS = [
  { key: 'exercise_id', label: 'Übung', type: 'applookup/select', targetEntity: 'uebungen', targetAppId: 'UEBUNGEN', displayField: 'name' },
  { key: 'date', label: 'Datum', type: 'date/date' },
  { key: 'weight_kg', label: 'Gewicht (kg)', type: 'number' },
  { key: 'reps', label: 'Wiederholungen', type: 'number' },
  { key: 'sets', label: 'Sätze', type: 'number' },
  { key: 'note', label: 'Notiz', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'uebungen', label: 'Übungen', pascal: 'Uebungen' },
  { key: 'pr_eintraege', label: 'PR-Einträge', pascal: 'PrEintraege' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'pr_eintraege': return (data as any).prEintraege as PrEintraege[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'pr_eintraege' && fieldKey === 'exercise_id') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'uebungen': return UEBUNGEN_FIELDS;
      case 'pr_eintraege': return PREINTRAEGE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'pr_eintraege': return {
        create: (fields: any) => LivingAppsService.createPrEintraegeEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updatePrEintraegeEntry(id, fields),
        remove: (id: string) => LivingAppsService.deletePrEintraegeEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'pr_eintraege' || dialogState?.entity === 'pr_eintraege') && (
        <PrEintraegeDialog
          open={createEntity === 'pr_eintraege' || dialogState?.entity === 'pr_eintraege'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'pr_eintraege' ? handleUpdate : (fields: any) => handleCreate('pr_eintraege', fields)}
          defaultValues={dialogState?.entity === 'pr_eintraege' ? dialogState.record?.fields : undefined}
      ...
      {viewState?.entity === 'pr_eintraege' && (
        <PrEintraegeViewDialog
          open={viewState?.entity === 'pr_eintraege'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'pr_eintraege', record: r }); }}
          uebungenList={(data as any).uebungen ?? []}
        />
      )}
